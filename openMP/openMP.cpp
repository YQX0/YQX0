...
#pragma omp parallel for num_threads(2)
	for (n = 0; n < 1000000; n++)  comput(A, B, C);   //parallel 2
	f = clock();
	duration = (double)(f - s) / CLOCKS_PER_SEC;
	printf("Parallel 2 :%f\n", duration);
	s = clock();
#pragma omp parallel for num_threads(4)
	for (n = 0; n < 1000000; n++) comput(A, B, C);   //parallel 4
	f = clock();
	duration = (double)(f - s) / CLOCKS_PER_SEC;
	printf("Parallel 4 :%f\n", duration);
	s = clock();
#pragma omp parallel for num_threads(8)
	for (n = 0; n < 1000000; n++)		comput(A, B, C); //parallel 8
	f = clock();
	duration = (double)(f - s) / CLOCKS_PER_SEC;
	printf("Parallel 8 :%f\n", duration);
	s = clock();
#pragma omp parallel for num_threads(16)
	for (n = 0; n < 1000000; n++) comput(A, B, C);    //parallel 16
	f = clock();
	duration = (double)(f - s) / CLOCKS_PER_SEC;
